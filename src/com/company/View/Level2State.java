...
public class Level2State extends LevelState implements Runnable {
    Thread rivalFactory=new Thread(this);
    private BossView bV=new BossView(this);
    ...
    private void testWin() {
        if(finish==true){
            ...
            if(LevelReader.getCurrentLevel()!=LevelReader.getSize()){
                ...
                File fileSave = new File("file/save.txt");
                fileSave.delete();
                gsm.setState(GameStateManager.MENUSTATE);
            }
        }
    ...
    private void checkAttack() {
        for (int ind=0;ind<p.getShootsSize();ind++)
            if(p.checkAttack(ind, bV.getRect())){
                if(bV.getLifeForce()>=0)
                    bV.setLifeForce(bV.getLifeForce()-1);
                if(bV.getLifeForce()==0) {
                    finish=true;
                    break;
                }
            }
        if(bV.checkAttack(p.getRect())) {
            JOptionPane.showMessageDialog(null,"Вы проиграли");
            gsm.setState(GameStateManager.MENUSTATE);
        }
    }
